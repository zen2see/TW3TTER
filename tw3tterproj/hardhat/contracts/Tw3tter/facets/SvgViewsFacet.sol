...
contract SvgViewsFacet is Modifiers {
    ///@notice Get the sideview svgs of an postget
    ///@dev Only valid for claimed postgets
    ///@param _tokenId The identifier of the postget to query
    ///@return ag_ An array of svgs, each one representing a certain perspective i.e front,left,right,back views respectively
    function getPostGetSideSvgs(uint256 _tokenId) public view returns (string[] memory ag_) {
        // 0 == front view
        // 1 == leftSide view
        // 2 == rightSide view
        // 3 == backSide view
        ...
        ag_ = new string[](4);
        require(s.postgets[_tokenId].status == LibPostGet.STATUS_POSTGET, "SvgFacet: PostGet not claimed");
        ag_[0] = SvgFacet(address(this)).getPostGetSvg(_tokenId);

        address collateralType = s.postgets[_tokenId].collateralType;
        int16[NUMERIC_TRAITS_NUM] memory _numericTraits = s.postgets[_tokenId].numericTraits;
        ...
        ag_[1] = string(getPostGetSideSvgLayers("left", collateralType, _numericTraits, _tokenId, hauntId, equippedWearables));
        ag_[1] = string(abi.encodePacked(viewBox, ag_[1], "</svg>"));

        ag_[2] = string(getPostGetSideSvgLayers("right", collateralType, _numericTraits, _tokenId, hauntId, equippedWearables));
        ag_[2] = string(abi.encodePacked(viewBox, ag_[2], "</svg>"));

        ag_[3] = string(getPostGetSideSvgLayers("back", collateralType, _numericTraits, _tokenId, hauntId, equippedWearables));
    ...
    function getPostGetSideSvgLayers(
        ...
        // aavagotchi body
        svg_ = LibSvg.getSvg(LibSvg.bytesToBytes32("postget-", _sideView), LibSvg.POSTGET_BODY_SVG_ID);
        ...
        //If tokenId is MAX_INT, we're rendering a Portal PostGet, so no wearables.
        if (_tokenId == type(uint256).max) {
            if (side == back) {
                svg_ = abi.encodePacked(
                    applySideStyles(details, _tokenId, equippedWearables),
                    LibSvg.getSvg(LibSvg.bytesToBytes32("postget-", _sideView), LibSvg.BACKGROUND_SVG_ID),
                    svg_
                );
            } else {
                svg_ = abi.encodePacked(
    ...
    //@notice Allow the sideview preview of an postget given the haunt id,a set of traits,wearables and collateral type
    ...
    function previewSidePostGet(
        ...
        //Front
        ag_[0] = SvgFacet(address(this)).previewPostGet(_hauntId, _collateralType, _numericTraits, equippedWearables);

        bytes memory viewBox = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">';

        //Left
        bytes memory svg_ = getPostGetSideSvgLayers("left", _collateralType, _numericTraits, type(uint256).max - 1, _hauntId, equippedWearables);
        ...
        //Right
        svg_ = getPostGetSideSvgLayers("right", _collateralType, _numericTraits, type(uint256).max - 1, _hauntId, equippedWearables);
        ...
        //Back
        svg_ = getPostGetSideSvgLayers("back", _collateralType, _numericTraits, type(uint256).max - 1, _hauntId, equippedWearables);
    ...
    function addBodyAndWearableSideSvgLayers(
        ...
        layers.hands = abi.encodePacked(svg_, LibSvg.getSvg(LibSvg.bytesToBytes32("postget-", _sideView), LibSvg.HANDS_SVG_ID));
