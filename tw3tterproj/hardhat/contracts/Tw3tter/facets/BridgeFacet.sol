...
contract BridgeFacet is Modifiers {
    ...
    event AddedPostGetBatch(address indexed owner, uint256[] tokenIds);
...
///@notice Allow the PostGet Diamond owner or Dao to change the childChain manager address
...
///@param _tokenIds An array containing the identifiers of the NFTs to withdraw
    function withdrawPostGetBatch(uint256[] calldata _tokenIds) external {
        address owner = LibMeta.msgSender();
        require(_tokenIds.length <= 20, "Bridge: exceeds withdraw limit for single transaction");
        for (uint256 i; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(owner == s.postgets[tokenId].owner, "BridgeFacet: Not owner of token");
            require(s.postgets[tokenId].locked == false, "BridgeFacet: Can't withdraw locked token");
            LibPostGet.transfer(owner, address(this), tokenId);
        }
    ...
    function deposit(address _user, bytes calldata _depositData) external {
        ...
        if (tokenType == ERC1155_TOKEN_TYPE) {
            ...
            for (uint256 i; i < tokenIds.length; i++) {
                ...
                require(address(this) == s.postgets[tokenId].owner, "Bridge: Not owner of token");
                LibPostGet.transfer(address(this), _user, tokenId);
            }
            emit AddedPostGetBatch(_user, tokenIds);
        }
    }
}