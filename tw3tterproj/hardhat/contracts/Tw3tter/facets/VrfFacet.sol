   ...
   |__________________________________*/
    ...
    function openPortals(uint256[] calldata _tokenIds) external {
        ...
        for (uint256 i; i < _tokenIds.length; i++) {
            ...
            require(s.postgets[tokenId].status == LibPostGet.STATUS_CLOSED_PORTAL, "PostGetFacet: Portal is not closed");
            require(owner == s.postgets[tokenId].owner, "PostGetFacet: Only postget owner can open a portal");
            require(s.postgets[tokenId].locked == false, "PostGetFacet: Can't open portal when it is locked");
    ...
    function drawRandomNumber(uint256 _tokenId) internal {
        s.postgets[_tokenId].status = LibPostGet.STATUS_VRF_PENDING;
        uint256 fee = s.fee;
    ...
    function tempFulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal {
        ...
        require(s.postgets[tokenId].status == LibPostGet.STATUS_VRF_PENDING, "VrfFacet: VRF is not pending");
        s.postgets[tokenId].status = LibPostGet.STATUS_OPEN_PORTAL;
    ...
    ///@notice Allow the postget diamond owner to change the vrf details
