...
import {LibAppStorage, Modifiers, ItemType, PostGet, ItemType, WearableSet, NUMERIC_TRAITS_NUM, EQUIPPED_WEARABLE_SLOTS, PORTAL_POSTGETS_NUM} from "../libraries/LibAppStorage.sol";
import {LibPostGet} from "../libraries/LibPostGet.sol";
...
contract ItemsFacet is Modifiers {
   ...
   |__________________________________*/
    ...
    ///@dev Only valid for claimed postgets
    ...
    function equippedWearables(uint256 _tokenId) external view returns (uint16[EQUIPPED_WEARABLE_SLOTS] memory wearableIds_) {
        wearableIds_ = s.postgets[_tokenId].equippedWearables;
    }
   ...
   |__________________________________*/
    ...
    ///@notice Allow the owner of a claimed postget to equip/unequip wearables to his postget
    ///@dev Only valid for claimed postgets
    ...
    ///@param _tokenId The identifier of the postget to make changes to
    ...
    function equipWearables(uint256 _tokenId, uint16[EQUIPPED_WEARABLE_SLOTS] calldata _wearablesToEquip)
        external
        onlyPostGetOwner(_tokenId)
        onlyUnlocked(_tokenId)
    {
        PostGet storage postget = s.postgets[_tokenId];
        require(postget.status == LibPostGet.STATUS_POSTGET, "LibPostGet: Only valid for AG");
        emit EquipWearables(_tokenId, postget.equippedWearables, _wearablesToEquip);

        address sender = LibMeta.msgSender();

        for (uint256 slot; slot < EQUIPPED_WEARABLE_SLOTS; slot++) {
            uint256 toEquipId = _wearablesToEquip[slot];
            uint256 existingEquippedWearableId = postget.equippedWearables[slot];
            ...
            postget.equippedWearables[slot] = uint16(toEquipId);
            ...
            if (existingEquippedWearableId != 0 && s.itemTypes[existingEquippedWearableId].canBeTransferred) {
                // remove wearable from PostGet and transfer item to owner
            ...
            if (toEquipId != 0) {
                ...
                require(LibPostGet.postgetLevel(postget.experience) >= itemType.minLevel, "ItemsFacet: AG level lower than minLevel");
                ...
                require(itemType.slotPositions[slot] == true, "ItemsFacet: Wearable can't be equipped in slot");
                {
                    ...
                    if (allowedCollaterals.length > 0) {
                        uint256 collateralIndex = s.collateralTypeIndexes[postget.collateralType];
                ...
                //Then check if this wearable is in the PostGets inventory
        ...
        LibPostGet.interact(_tokenId);
    }

    ///@notice Allow the owner of an NFT to use multiple consumable items for his postget
    ///@dev Only valid for claimed postgets
    ///@dev Consumables can be used to boost kinship/XP of an postget
    ///@param _tokenId Identtifier of postget to use the consumables on
    ...
    function useConsumables(
    ...
    ) external onlyUnlocked(_tokenId) onlyPostGetOwner(_tokenId) {
        require(_itemIds.length == _quantities.length, "ItemsFacet: _itemIds length != _quantities length");
        require(s.postgets[_tokenId].status == LibPostGet.STATUS_POSTGET, "LibPostGet: Only valid for AG");
        ...
        for (uint256 i; i < _itemIds.length; i++) {
            ...
            if (itemType.kinshipBonus > 0) {
                uint256 kinship = (uint256(int256(itemType.kinshipBonus)) * quantity) + s.postgets[_tokenId].interactionCount;
                s.postgets[_tokenId].interactionCount = kinship;
            } else if (itemType.kinshipBonus < 0) {
                uint256 kinshipBonus = LibAppStorage.abs(itemType.kinshipBonus) * quantity;
                if (s.postgets[_tokenId].interactionCount > kinshipBonus) {
                    s.postgets[_tokenId].interactionCount -= kinshipBonus;
                } else {
                    s.postgets[_tokenId].interactionCount = 0;
                }
            }

            {
                ...
                if (boost) {
                    s.postgets[_tokenId].lastTemporaryBoost = uint40(block.timestamp);
                    s.postgets[_tokenId].temporaryTraitBoosts = itemType.traitModifiers;
                }
            }

            //Increase experience
            if (itemType.experienceBonus > 0) {
                uint256 experience = (uint256(itemType.experienceBonus) * quantity) + s.postgets[_tokenId].experience;
                s.postgets[_tokenId].experience = experience;
            }
