...
import {LibPostGet, PostGetInfo} from "../libraries/LibPostGet.sol";
...
contract ERC721MarketplaceFacet is Modifiers {
    ...
    ///@notice Get an postget listing details through an identifier
    ...
    ///@return postgetInfo_ A struct containing details about the postget
    function getPostGetListing(uint256 _listingId) external view returns (ERC721Listing memory listing_, PostGetInfo memory postgetInfo_) {
        listing_ = s.erc721Listings[_listingId];
        require(listing_.timeCreated != 0, "ERC721Marketplace: ERC721 listing does not exist");
        postgetInfo_ = LibPostGet.getPostGet(listing_.erc721TokenId);
    }
    ...
    ///@param _category Category of listings to query // 0 == portal, 1 == vrf pending, 1 == open portal, 2 == PostGet.
    ...
    struct PostGetListing {
        ERC721Listing listing_;
        PostGetInfo postgetInfo_;
    }

    ///@notice Query a certain amount of postget listings created by an address based on their category and sortings
    ///@param _owner Creator of the listings to query
    ///@param _category Category of listings to query  // 0 == portal, 1 == vrf pending, 1 == open portal, 2 == PostGet.
    ///@param _sort Sortings of listings to query // "listed" or "purchased"
    ///@param _length How many postget listings to return
    ///@return listings_ An array of structs, each struct containing details about each listing being returned
    function getOwnerPostGetListings(
        address _owner,
        uint256 _category,
        string memory _sort,
        uint256 _length // how many items to get back or the rest available
    ) external view returns (PostGetListing[] memory listings_) {
        uint256 listingId = s.erc721OwnerListingHead[_owner][_category][_sort];
        listings_ = new PostGetListing[](_length);
        uint256 listIndex;
        for (; listingId != 0 && listIndex < _length; listIndex++) {
            ERC721Listing memory listing = s.erc721Listings[listingId];
            listings_[listIndex].listing_ = listing;
            listings_[listIndex].postgetInfo_ = LibPostGet.getPostGet(listing.erc721TokenId);
            listingId = s.erc721OwnerListingListItem[listingId].childListingId;
        }
        assembly {
            mstore(listings_, listIndex)
    ...
    function getERC721Listings(
        uint256 _category, // 0 == portal, 1 == vrf pending, 1 == open portal, 2 == PostGet.
    ...
    ///@notice Query a certain amount of postget listings
    ///@param _category Category of listings to query // 0 == portal, 1 == vrf pending, 1 == open portal, 2 == PostGet.
    ///@param _sort Sortings of listings to query
    ///@param _length How many listings to return
    ///@return listings_ An array of structs, each struct containing details about each listing being returned
    function getPostGetListings(
        uint256 _category, // 0 == portal, 1 == vrf pending, 1 == open portal, 2 == PostGet
        string memory _sort, // "listed" or "purchased"
        uint256 _length // how many items to get back or the rest available
    ) external view returns (PostGetListing[] memory listings_) {
    ...
    struct Category {
        address erc721TokenAddress;
        uint256 category; // 0,1,2,3 == PostGet diamond, 4 == Realm diamond.
        // uint256 status; can add this in later if necessary
    }

    ///@notice Allow the postget diamond owner or DAO to set the category details for different types of ERC721 NFTs
    ...
    ///@return category_ Category of the NFT // 0 == portal, 1 == vrf pending, 2 == open portal, 3 == PostGet 4 == Realm.
    function getERC721Category(address _erc721TokenAddress, uint256 _erc721TokenId) public view returns (uint256 category_) {
        ...
        if (_erc721TokenAddress != address(this)) {
            ...
            category_ = s.postgets[_erc721TokenId].status; // 0 == portal, 1 == vrf pending, 2 == open portal, 3 == PostGet
        }
    }
    ...
    ///@param _priceInWei The cost price of the NFT in $POGT
    ...
    function addERC721Listing(
        ...
        require(_priceInWei >= 1e18, "ERC721Marketplace: price should be 1 POGT or larger");
        ...
        require(category != LibPostGet.STATUS_VRF_PENDING, "ERC721Marketplace: Cannot list a portal that is pending VRF");
        ...
        //Lock PostGets when listing is created
        if (_erc721TokenAddress == address(this)) {
            s.postgets[_erc721TokenId].locked = true;
        }

        // Check if there's a publication fee and
        // transfer the amount to burn address
        if (s.listingFeeInWei > 0) {
            LibERC20.transferFrom(s.pogtContract, owner, address(0xFFfFfFffFFfffFFfFFfFFFFFffFFFffffFfFFFfF), s.listingFeeInWei);
        }
    }
    ...
    function executeERC721Listing(uint256 _listingId) external {
        ...
        require(IERC20(s.pogtContract).balanceOf(buyer) >= priceInWei, "ERC721Marketplace: not enough POGT");
        ...
        LibERC20.transferFrom(s.pogtContract, buyer, s.artists, artistsShare);
        LibERC20.transferFrom(s.pogtContract, buyer, s.daoTreasury, daoShare);
        LibERC20.transferFrom(s.pogtContract, buyer, seller, transferAmount);
        //AGIP6 adds on 0.5%
        LibERC20.transferFrom((s.pogtContract), buyer, s.rarityFarming, playerRewardsShare);

        if (listing.erc721TokenAddress == address(this)) {
            s.postgets[listing.erc721TokenId].locked = false;
            LibPostGet.transfer(seller, buyer, listing.erc721TokenId);
        } else {
            // External contracts
