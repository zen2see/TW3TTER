...
contract EscrowFacet is Modifiers {
    ...
    function gotchiEscrow(uint256 _tokenId) public view returns (address) {
        return s.postgets[_tokenId].escrow;
    }

    ///@notice Allow the deposit of an ERC20 token to the escrow contract of a claimed postget
    ///@dev Will throw if token being deposited is same as collateral token for the postget
    ...
    function depositERC20(
        ...
        address escrow = s.postgets[_tokenId].escrow;
    ...
    ///@notice Allow the deposit of multiple ERC20 tokens to the escrow contract of a multiple claimed postgets
    ///@dev Will throw if one of the tokens being deposited is same as collateral token for the corresponding postget
    ...
    ///@notice Allow the deposit of GHST into the escrow of multiple postgets
    ///@param _tokenIds An array containing the identifiers of the NFTs receiving GHST
    ///@param _values An array containing the amounts of ERC20 tokens to deposit into each postget
    ...
    function batchDepositGHST(uint256[] calldata _tokenIds, uint256[] calldata _values) external {
        ...
        for (uint256 index = 0; index < _tokenIds.length; index++) {
            uint256 tokenId = _tokenIds[index];
            uint256 value = _values[index];
            address escrow = s.postgets[tokenId].escrow;
            require(escrow != address(0), "EscrowFacet: Does not have an escrow");
    ...
    ///@notice Query the balance of any ERC20 token being hekd in the escrow of an postget
    ...
    ///@notice Allow the owner of the postget to transfer out any ERC20 token in the escrow to an external address
    ...
    function transferEscrow(
    ...
    ) external onlyPostGetOwner(_tokenId) onlyUnlocked(_tokenId) {
        address escrow = s.postgets[_tokenId].escrow;
        address collateralType = s.postgets[_tokenId].collateralType;
        require(escrow != address(0), "EscrowFacet: Does not have an escrow");
        ...
        if (IERC20(_erc20Contract).allowance(escrow, address(this)) < _transferAmount) {
            CollateralEscrow(escrow).approvePostGetDiamond(_erc20Contract);
        }
