...
uint256 constant PORTAL_POSTGETS_NUM = 10;
...
struct PostGet {
    uint16[EQUIPPED_WEARABLE_SLOTS] equippedWearables; //The currently equipped wearables of the PG
    ...
    uint256 interactionCount; //How many times the owner of this PG has interacted with it.
    address collateralType;
    uint40 claimTime; // The block timestamp when this PGtkn was claimed
    uint40 lastTemporaryBoost;
    uint16 hauntId;
    address owner;
    uint8 status; // 0 == portal, 1 == VRF_PENDING, 2 == open portal, 3 == PG
    uint40 lastInteracted; // The last time this PG was interacted with
    bool locked;
    address escrow; // The escrow address this PG manages.
}
...
struct ItemType {
    ...
    uint256 pogtPrice; //How much GHST this item costs
...
struct PostGetCollateralTypeInfo {
...
struct AppStorage {
    mapping(address => PostGetCollateralTypeInfo) collateralTypeInfo;
    ...
    mapping(uint256 => PostGet) postgets;
    ...
    mapping(string => bool) postgetNamesUsed;
...
contract Modifiers {
    ...
    modifier onlyPostGetOwner(uint256 _tokenId) {
        require(LibMeta.msgSender() == s.postgets[_tokenId].owner, "LibAppStorage: Only postget owner can call this function");
        _;
    }
    modifier onlyUnlocked(uint256 _tokenId) {
        require(s.postgets[_tokenId].locked == false, "LibAppStorage: Only callable on unlocked postgets");
        _;
    }
