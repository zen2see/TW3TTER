...
export const TwitterContext = createContext("Default")
...
export const TwitterProvider = ({ children }) => {
  ...
  const checkIfWalletIsConnected = async () => {
    // if (!(window as any).ethereum.request) return setAppStatus('noMetaMask')
    if (!(window).ethereum.request) return setAppStatus('noMetaMask')
    try {
      const addressArray = await (window).ethereum.request.ethereum.request({
        method: 'eth_accounts',
      })
